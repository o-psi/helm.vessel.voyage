...
use uuid::Uuid;
use crate::vessel::{VesselRequest, VesselResponse, VesselEvent, VesselEventRequest};
...
pub enum ClientFrame {
    Command { request_id: Uuid, request: VesselRequest },
    Subscribe { request_id: Uuid, request: VesselEventRequest },
    Unsubscribe { subscription_id: Uuid },
    ReverseReply { request_id: Uuid, reply: ReverseReply },
}

#[derive(Clone, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case", deny_unknown_fields)]
pub enum ServerFrame {
    Hello { protocol: u32, socket_id: Uuid, vessel_id: Uuid },
    Reply { request_id: Uuid, response: VesselResponse },
    Subscribed { request_id: Uuid },
    Event { subscription_id: Uuid, event: VesselEvent },
    ReverseRequest { request_id: Uuid, request: ReverseRequest },
}
