...
def explicit_upgrade(root):
    ...
    try:
        ...
        with sqlite3.connect(case.database) as database:
            database.executescript('DROP TABLE remote_withdrawal; DROP TABLE remote_cleanup_attestations; DROP TABLE remote_text; DROP TABLE remote_receipts; DROP TABLE remote_events; DROP TABLE remote_tools; DROP TABLE remote_session; DROP TABLE local_tool_reconciliations; UPDATE attachment_schema SET version=5;')
        ...
        assert case.sql('SELECT version FROM attachment_schema') == [(8,)]
