...
impl Supervisor {
    pub(super) async fn resolve_start(
        ...
        self.resolve_start_original(command_id, session_id, workspace, config_path, original).await
    }
    pub(super) async fn resolve_start_original(&self, command_id: Uuid, session_id: Uuid, workspace: PathBuf, config_path: Option<PathBuf>, original: VesselCommand) -> Result<Value> {
