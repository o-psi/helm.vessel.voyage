...
func main() {
    ...
    err:=sdk.Serve(definitions,[]string{"execute"},func(ctx context.Context,c *sdk.Call,kind,name string,args json.RawMessage)(any,error){
        ...
        var input struct { Mode string `json:"mode"`; Text string `json:"text"` }
        if err:=json.Unmarshal(args,&input);err!=nil{return nil,err}
        if kind=="command" && name=="echo_command" {return map[string]string{"text":input.Text},nil}
        if kind!="tool" || name!="probe" {return nil,errors.New("unsupported")}
        switch input.Mode {
        case "echo": return map[string]string{"text":input.Text},nil
        case "block":
            if err:=c.Progress("Waiting for cancellation");err!=nil{return nil,err}
            <-ctx.Done(); return nil,ctx.Err()
        case "crash": os.Exit(7)
        case "flood":
            _,err:=os.Stdout.WriteString(strings.Repeat("x",1024*1024+2));return nil,err
        case "held_child":
            _,err:=os.StartProcess("/extension",[]string{"/extension","--held-child"},&os.ProcAttr{Files:[]*os.File{os.Stdin,os.Stdout,os.Stderr}})
            if err!=nil{return nil,err};return map[string]bool{"child_started":true},nil
        case "isolation":
            _,fileErr:=os.ReadFile("/etc/passwd")
            fd,netErr:=syscall.Socket(syscall.AF_INET,syscall.SOCK_STREAM,0)
            if netErr==nil{syscall.Close(fd)}
