...
**Source-only, uncompiled and untested during the #213 build pause.** See the
...
```sh
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags='-s -w' -o transform ./cmd/transform
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags='-s -w' -o read-text ./cmd/read
```
