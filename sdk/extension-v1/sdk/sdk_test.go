...
func TestStrictJSON(t *testing.T) {
	for _, input := range []string{`{"a":1,"a":2}`, `{} {}`, strings.Repeat("[", 49) + "0" + strings.Repeat("]", 49), "[" + strings.Repeat("0,", 4096) + "0]"} {
		if _, err := strict([]byte(input)); err == nil {
			t.Fatal("accepted invalid JSON")
		}
	}
	if _, err := strict([]byte(`{"a":[1,null,true,"text"]}`)); err != nil {
		t.Fatal(err)
	}
}
...
func TestVariantFields(t *testing.T) {
	if !validFields([]byte(`{"type":"shutdown"}`)) {
		t.Fatal("refused shutdown")
	}
	if validFields([]byte(`{"type":"shutdown","arguments":{}}`)) {
		t.Fatal("accepted cross-variant field")
	}
	if validFields([]byte(`{"type":"cancel"}`)) {
