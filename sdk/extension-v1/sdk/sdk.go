...
func (c *Call) Progress(text string) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.closed || len(c.readGate) > 0 || !utf8.ValidString(text) || len(text) > 4096 || c.progressCount >= 256 {
		return ErrProtocol
	}
	now := time.Now()
	for len(c.progress) > 0 && now.Sub(c.progress[0]) >= time.Second {
		c.progress = c.progress[1:]
	}
	if len(c.progress) >= 10 {
		return ErrProtocol
	}
	c.progress = append(c.progress, now)
	c.progressCount++
...
func (c *Call) Read(ctx context.Context, path string, offset uint64, maxBytes int) (string, error) {
	if maxBytes < 1 || maxBytes > 65536 || len(path) < 1 || len(path) > 4096 {
		return "", ErrProtocol
	}
	select {
	...
	c.mu.Lock()
	if c.closed || !c.canRead || c.requests >= 32 {
		c.mu.Unlock()
		return "", ErrDenied
	}
	c.requests++
	...
	case reply := <-c.replies:
		if reply.Request != request || reply.Invocation != c.identity.Invocation {
			return "", ErrProtocol
		}
		if reply.Type == "host_error" {
			return "", ErrDenied
		}
		if reply.Type != "host_result" || len(reply.Text) > maxBytes {
			return "", ErrProtocol
		}
...
func serve(reader io.Reader, writer io.Writer, definitions json.RawMessage, capabilities []string, handler Handler) error {
	...
	if err != nil || init.Type != "initialize" || init.Protocol != 1 || init.Identity == nil ||
		!reflect.DeepEqual(got, pinned) || !reflect.DeepEqual(init.Capabilities, capabilities) {
		return ErrProtocol
	}
	if err = out.send(map[string]any{"type": "initialized", "protocol": 1, "identity": init.Identity,
		"definitions": definitions, "capabilities": capabilities}); err != nil {
		return err
	}
	invoke, err := receive()
	if err != nil {
	...
	for {
		...
		case r := <-done:
			...
			if !finished {
				...
				if r.err != nil {
					reply = map[string]any{"type": "error", "invocation": c.identity.Invocation, "code": "failed"}
				}
		...
		case m := <-frames:
			switch m.Type {
			case "cancel":
				if m.Invocation != c.identity.Invocation {
					return ErrProtocol
				}
				cancel()
				c.mu.Lock()
...
func readFrame(r *bufio.Reader) ([]byte, error) {
	...
	for {
		...
		if len(frame)+len(part) > MaxFrame+1 {
