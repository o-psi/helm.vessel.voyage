...
async function refresh(){
  ...
  const key=JSON.stringify([state.tabs,state.selected,state.origins,state.prompts,state.uploads,state.downloads,state.dialog]);
...
function sendInput(b){if(!interactive())return;const generation=localGeneration,epoch=state.epoch;inputQueue=inputQueue.then(()=>{if(generation!==localGeneration)return;return api({...b,epoch});}).catch(fail);}
...
viewport.onpointerdown=e=>{if(!interactive())return;e.preventDefault();viewport.focus();viewport.setPointerCapture(e.pointerId);sendInput({op:'pointer',kind:'down',button:e.button,click_count:e.detail===2?2:1,...coords(e)});};
...
window.addEventListener('blur',()=>{localGeneration++;});
