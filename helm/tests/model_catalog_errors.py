...
def main():
    ...
    rows=[]
    for code,body,label in [(401,b'SECRET-provider-diagnostic','rejected model-catalogue authentication'),(200,b'{"data":"SECRET-malformed"}','could not parse'),(429,b'SECRET-rate-limit','rate-limited')]:
        ...
        try:
            ...
            assert 'SECRET' not in ui.screen.text();assert f.provider.catalog_calls>0;assert len(f.provider.bodies)==0;assert not f.snapshot(sid)['messages']
