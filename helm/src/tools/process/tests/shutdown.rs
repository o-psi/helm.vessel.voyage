...
async fn empty_shutdown_is_repeatable_and_closes_shared_admission() {
    ...
    assert!(
        clone
            .execute(json!({"action":"start","command":"echo forbidden"}), &ctx)
            .await
            .is_err()
    );
}
...
async fn cancelled_shutdown_waiter_does_not_reopen_admission() {
    ...
    tokio::time::timeout(Duration::from_secs(1), async {
        while !tool.shutting_down.load(Ordering::SeqCst) {
    ...
    drop(lock);
    assert!(
        tool.execute(json!({"action":"start","command":"echo forbidden"}), &ctx)
            .await
            .is_err()
    );
    assert!(
...
async fn approval_finishing_after_shutdown_cannot_spawn() {
    ...
    let task = tokio::spawn(async move {
        copy.execute(
            json!({"action":"start","command":"echo should-not-start"}),
            &ctx,
        )
        .await
    });
...
"#,
    ...
    owned
        .execute(
            json!({"action":"start","command":"exec python3 child.py"}),
            &ctx,
        )
        .await
        .unwrap();
    ...
    .await
    .unwrap();
    let pids: Vec<u32> = std::fs::read_to_string(dir.path().join("ready"))
        .unwrap()
        .split_whitespace()
...
async fn ordinary_status_reads_do_not_reap_session_leader_before_shutdown() {
    ...
    let tool = ProcessTool::default();
    tool.execute(
        json!({"action":"start","command":"exit 0"}),
        &context(dir.path()),
    )
    .await
    .unwrap();
    tokio::time::timeout(Duration::from_secs(3), async {
        while tool.metadata().unwrap()[0].state == "running" {
            tokio::time::sleep(Duration::from_millis(10)).await;
        }
    })
    .await
    ...
    assert!(
        ...
        "leader identity stays reserved until explicit cleanup"
    );
    assert!(
        tool.shutdown(Duration::from_secs(3))
            .await
            .observation_complete
    );
