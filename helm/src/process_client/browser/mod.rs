...
pub async fn run_cli(args: &BrowserArgs) -> Result<()> {
    match args.command {
        ...
        BrowserCommand::Status => {
            ...
            println!("Use Helm F6 or /browser on a selected voyage. Sharing and private return are explicit local companion operations.");
            Ok(())
        }
    }
}
...
impl Default for Status {
    fn default() -> Self { Self { summary: "Starting local browser; not shared".into(), launcher: None, finished: false } }
}
#[derive(Clone, Copy)]
pub(crate) enum Control { Human, Private }
...
impl Handle {
    pub fn start(client: Client, session: Uuid, incarnation: Uuid, label: String) -> Self {
        ...
        let job = tokio::spawn(async move {
            let result = runner::run(client, session, incarnation, label, input, cancelled, &state).await;
            state.send_modify(|s| {
                s.finished = true;
        ...
        Self { state: status, control, stop, job: Some(job) }
    }
    pub fn control(&self, value: Control) -> Result<()> {
        self.control.try_send(value).context("Local browser control queue unavailable")
    }
    pub fn stop(&self) { self.stop.cancel(); }
    pub fn finished(&self) -> bool { self.job.as_ref().is_none_or(|job| job.is_finished()) }
    pub async fn finish(&mut self) -> Result<()> {
        self.stop.cancel();
        if let Some(job) = self.job.take() { job.await.context("Local browser cleanup task failed")??; }
        Ok(())
    }
}
...
pub(super) fn launcher(root: &std::path::Path, url: &str) -> Result<PathBuf> {
    ...
    ensure!(url.scheme() == "http" && url.host_str() == Some("127.0.0.1") && url.port().is_some()
        && url.username().is_empty() && url.password().is_none() && url.query().is_none()
        && url.path() == "/" && url.fragment().is_some(), "Invalid local companion address");
    let escaped = url.as_str().replace('&', "&amp;").replace('"', "&quot;").replace('<', "&lt;");
    let text = format!("<!doctype html><meta charset=\"utf-8\"><meta name=\"referrer\" content=\"no-referrer\"><meta http-equiv=\"refresh\" content=\"0;url={escaped}\"><title>Local Helm browser</title><a href=\"{escaped}\">Open local browser companion</a>");
...
pub(crate) async fn open_launcher(path: PathBuf) -> Result<()> {
    ensure!(path.is_absolute(), "Local browser launcher must be absolute");
    #[cfg(target_os = "linux")]
    {
        let mut child = tokio::process::Command::new("xdg-open").arg(&path)
        ...
        match tokio::time::timeout(Duration::from_secs(5), child.wait()).await {
            Ok(result) => ensure!(result?.success(), "Could not open companion; open its private local launcher file manually"),
            Err(_) => { let _ = child.start_kill(); child.wait().await?; anyhow::bail!("Companion opener did not finish; local browser remains unshared"); }
        }
        Ok(())
    }
    #[cfg(not(target_os = "linux"))]
    { let _ = path; anyhow::bail!("Local browser desktop integration currently requires Linux; native support is not verified") }
}
