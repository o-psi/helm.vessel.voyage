...
const FILES: &[(&str, &str)] = &[
    ...
    (
        "security.mjs",
        include_str!("../../../browser/security.mjs"),
    ),
    ...
    (
        "package.json",
        include_str!("../../../browser/package.json"),
    ),
    (
        "package-lock.json",
        include_str!("../../../browser/package-lock.json"),
    ),
];
...
pub(super) fn root() -> Result<PathBuf> {
    ...
    let parent = base
        .parent()
        .context("browser storage parent unavailable")?;
    ...
    for (name, content) in FILES {
        match private.read_bounded(name, 65_536)? {
            Some(bytes) => ensure!(
                ...
                "Installed browser adapter differs from the bundled version; refusing replacement"
            ),
...
pub(super) fn executable() -> Result<PathBuf> {
    ...
    if let Some(path) = std::env::var_os("HELM_BROWSER_CHROMIUM") {
        let path = PathBuf::from(path);
            ...
            "HELM_BROWSER_CHROMIUM must name an absolute local Chromium executable"
        );
        return Ok(path);
    }
    ...
    anyhow::bail!(
        "Install Chromium locally or set HELM_BROWSER_CHROMIUM to its absolute executable path"
    )
}
pub(super) fn installed(path: &Path) -> bool {
    path.join("node_modules/playwright-core/package.json")
        .is_file()
        && path.join("installed.json").is_file()
}
pub(super) async fn setup() -> Result<()> {
    ...
    let _lock = private.lock()?;
    println!(
        "Installing the bundled, lockfile-pinned local browser adapter dependency. Browser and provider credentials are not involved."
    );
    ...
    let mut child = command
        .spawn()
        .context("Install Node.js and npm before running browser setup")?;
    let status = match tokio::time::timeout(Duration::from_secs(120), child.wait()).await {
        ...
        Err(_) => {
            ...
            tokio::time::timeout(Duration::from_secs(5), child.wait())
                .await
                .context("Browser dependency installation cleanup not observed")??;
            anyhow::bail!("Browser dependency installation timed out; no browser was shared");
        }
    };
    ensure!(
        status.success(),
        "Browser dependency installation failed; no browser was shared"
    );
    private.publish("installed.json", br#"{"version":1}"#)?;
    println!(
        "Local browser adapter installed. {}",
        if executable().is_ok() {
            "Chromium found. In Helm select a voyage and press F6 or use /browser."
        } else {
            "Install Chromium or set HELM_BROWSER_CHROMIUM to an absolute path before use."
        }
    );
    Ok(())
}
