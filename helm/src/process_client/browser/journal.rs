...
pub(super) fn offer(root: &Path, client: &Client, binding: &BrowserBinding) -> Result<()> {
    Directory::open_existing(root)?.publish(
        "binding.json",
...
pub(super) fn record(root: &Path, record: &Dispatch) -> Result<()> {
    Directory::open_existing(root)?.publish(
        &format!("action-{}.json", record.request_id),
...
pub(crate) async fn reconcile(client: Client, session: Uuid, incarnation: Uuid) -> Result<String> {
    ...
    for root in resources()? {
        ...
        let Some(bytes) = directory.read_bounded("binding.json", 65_536)? else {
            continue;
        };
        let resource: Resource = serde_json::from_slice(&bytes)?;
        if resource.version != 1
            || resource.connection_id != client.id()
            || resource.binding.session_id != session
        {
            continue;
        }
        let closed = directory
            .read_bounded("cleanup.json", 1024)?
            .is_some_and(|b| b == br#"{"observed":true}"#);
        for entry in std::fs::read_dir(&root)? {
            ...
            let bytes = directory
                .read_bounded(name, 65_536)?
                .context("Local browser dispatch record disappeared")?;
            let record: Dispatch = serde_json::from_slice(&bytes)?;
            if record.local_dispatch_possible && !closed {
                continue;
            }
            ensure!(
                record.binding.session_id == session
                    && record.binding.executor_id == resource.binding.executor_id,
                "Local browser evidence identity mismatch"
            );
            let operation = BrowserOperation::Cleanup {
            ...
            client
                .voyage(session, incarnation, VoyageCommand::Browser { operation })
                .await?;
            count += 1;
            ensure!(
                count <= 1024,
                "Browser reconciliation limit reached; inspect retained resources before another batch"
            );
        }
    }
    Ok(format!(
        "Reconciled {count} browser cleanup records from positive local evidence. No browser effects were replayed or reported successful. Unobserved resources remain fenced."
    ))
}
fn resources() -> Result<Vec<PathBuf>> {
    ...
    for entry in std::fs::read_dir(super::assets::root()?)? {
            ...
            .is_some_and(|n| n.starts_with("session-"))
        {
                ...
                roots.len() < 1024,
                "Local browser resource retention limit reached; review retained profiles"
            );
