...
impl App {
    pub(in crate::process_client::ui) fn new_draft_input(&mut self, event: &Event) -> Result<bool> {
        ...
        if let Some(key) = key {
            ...
            if key.code == KeyCode::F(1) {
                self.status = "Draft commands: /model NAME, /access MODE, /workspace PATH, /new [PATH], /discard. Enter sends; Tab changes view. First sends recover automatically.".into();
                return Ok(true);
            }
    ...
    fn draft_command(&mut self, id: Uuid, text: &str) -> Result<()> {
        ...
        if text == "/help" {
            self.status = "Draft commands: /model NAME, /access read-only|approval|unrestricted, /workspace PATH, /new [PATH], /discard. Enter sends; Tab changes view. First sends recover automatically.".into();
        } else {
            let draft = self.new_drafts.get_mut(&id).context("draft unavailable")?;
            anyhow::ensure!(
