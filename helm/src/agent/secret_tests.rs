use super::*;
use crate::{model::{ModelResponse, Role, ToolCall}, provider::ProviderError, workflow::secrets::SecretInputs};
use std::sync::Mutex;
...
impl Provider for PrivateCall {
    async fn complete(&self, request: ModelRequest) -> Result<ModelResponse, ProviderError> {
        ...
        Ok(ModelResponse { message, usage: Usage::default() })
    }
}
fn bindings(id: uuid::Uuid) -> crate::workflow::secrets::RunBindings {
    ...
    SecretInputs::collect(&doc, vec![("token".into(), "private-秘密-value".into())]).unwrap().bind(id).unwrap()
}
...
async fn secret_binding_fences_checkpoint_without_completion_scope_before_provider_dispatch() {
    ...
    let result = agent.run_checkpointed_scoped_with_workflow_secrets(vec![], "public reference".into(), CancellationToken::new(), None, &cp, "test".into(), None, Some(bindings(uuid::Uuid::new_v4()))).await;
...
async fn secret_binding_is_run_local_and_never_enters_requests_or_canonical_history() {
    ...
    let result = agent.run_checkpointed_scoped_with_workflow_secrets(vec![], "Use the public token reference".into(), CancellationToken::new(), None, &cp, "test".into(), None, Some(bindings(id))).await.unwrap();
    let tool = result.messages.iter().find(|m| m.role == Role::Tool).unwrap();
    assert_eq!(serde_json::from_str::<serde_json::Value>(&tool.content).unwrap(), serde_json::json!({"status":"exited","code":0}));
    let snapshot = cp.snapshot_for_finish().await;
    for text in [serde_json::to_string(&snapshot).unwrap(), serde_json::to_string(&result.messages).unwrap(), serde_json::to_string(&*requests.lock().unwrap()).unwrap()] {
        assert!(!text.contains("private-秘密-value"));
    }
    let later = agent.run(vec![], "Try the same reference again".into()).await.unwrap();
    assert!(later.messages.iter().any(|m| m.role == Role::Tool && m.content.contains("not available")));
