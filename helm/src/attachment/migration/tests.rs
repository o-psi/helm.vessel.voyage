...
async fn legacy_journal_requires_explicit_upgrade_before_source_transition() {
    ...
    let db = rusqlite::Connection::open(fixture.journal.join("journal.sqlite3")).unwrap();
    db.execute_batch("DROP TABLE local_cleanup_obligations; DROP TABLE local_cancel_intents; DROP TABLE steering; DROP TABLE imports; UPDATE attachment_schema SET version=2 WHERE id=1;")
        .unwrap();
    assert!(
        fixture
            .transfer()
