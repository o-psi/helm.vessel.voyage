...
fn cli_environment_source_is_explicit_and_preview_never_reads_it() {
    ...
    let definition = Definition { scope: Scope::User, digest: "a".repeat(64), document: super::parse(b"schema_version=1\nid='private-cli'\nversion='1'\ndescription='Private input'\nprompt='Use {{token}}'\n[parameters.token]\ntype='string'\nsecret=true\nrequired=true\n").unwrap() };
    let args = || InputArgs {
        selection: Selection {
            id: "private-cli".into(),
            scope: None,
    ...
    let run = super::prepare_inputs(&definition, args(), true, |name| {
        assert_eq!(name, "OPERATOR_TOKEN");
        Ok("q秘密".into())
    })
    .unwrap();
    assert_eq!(
        run.secrets.names().into_iter().collect::<Vec<_>>(),
        ["token"]
    );
    ...
    for refs in [
        vec!["unknown=TOKEN"],
        vec!["token=BAD-VALUE"],
        vec!["token=TOKEN", "token=OTHER"],
    ] {
        let mut input = args();
        input.secret_env = refs.into_iter().map(String::from).collect();
        assert!(
            super::prepare_inputs(&definition, input, true, |_| panic!(
                "invalid reference read environment"
            ))
            .is_err()
        );
    }
    let mut input = args();
    input.inputs = vec!["token=DO-NOT-ECHO".into()];
    let error = super::prepare_inputs(&definition, input, true, |_| {
        panic!("invalid ordinary input read secret")
    })
    .err()
    .unwrap()
    .to_string();
