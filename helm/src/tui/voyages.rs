...
mod tests {
    use super::*;
    fn press(panel: &mut Panel, code: KeyCode) {
        panel.key(KeyEvent::new(code, KeyModifiers::NONE));
    }
    fn choice(label: &str) -> HelmChoice {
        HelmChoice {
            id: Uuid::new_v4(),
            label: label.into(),
    ...
    fn setup() -> (Panel, Uuid, Uuid) {
        let a = choice("alpha");
        let b = choice("beta");
        let ids = (a.id, b.id);
        let mut panel = Panel::new();
        panel.set_helms(vec![a, b], None);
        panel.open();
        (panel, ids.0, ids.1)
    }
    #[test]
    fn complete_review_requires_explicit_scope_and_coordinator() {
        let (mut panel, a, _) = setup();
        press(&mut panel, KeyCode::Enter);
        assert!(panel.error.is_some());
        panel.paste("Investigation");
        press(&mut panel, KeyCode::Enter);
        panel.paste("Find root cause");
        press(&mut panel, KeyCode::Enter);
        press(&mut panel, KeyCode::Enter);
        assert!(panel.error.is_some());
    ...
    #[test]
    fn search_refresh_and_missing_entries_never_change_scope() {
        let (mut panel, a, b) = setup();
        panel.step = Step::Helms;
        press(&mut panel, KeyCode::Char(' '));
        panel.paste("beta");
        assert_eq!(panel.focused, Some(b));
        assert_eq!(panel.selected, vec![a]);
        press(&mut panel, KeyCode::Char(' '));
        let beta = panel.helms[1].clone();
        panel.set_helms(vec![beta], Some("offline".into()));
        assert_eq!(panel.selected, vec![a, b]);
        panel.key(KeyEvent::new(KeyCode::Char('u'), KeyModifiers::CONTROL));
        assert_eq!(panel.selected, vec![b]);
    }
    #[test]
    fn unicode_edit_paste_limits_and_cancel_preserve_draft() {
        let (mut panel, _, _) = setup();
        panel.paste("山😀\n\x1b");
        assert_eq!(panel.name.text, "山😀");
        press(&mut panel, KeyCode::Left);
        press(&mut panel, KeyCode::Backspace);
        assert_eq!(panel.name.text, "😀");
        panel.paste(&"界".repeat(200));
        assert!(panel.name.text.len() <= 256);
        let draft = panel.draft();
        press(&mut panel, KeyCode::Esc);
        panel.open();
        assert_eq!(panel.draft(), draft);
        panel.key(KeyEvent::new(KeyCode::Char('c'), KeyModifiers::CONTROL));
        assert!(!panel.is_open());
        assert!(panel.take_ready().is_none());
    }
    #[test]
    fn deselect_coordinator_requires_new_explicit_choice() {
        let (mut panel, a, _) = setup();
        panel.step = Step::Helms;
        panel.selected = vec![a];
        panel.coordinator = Some(a);
        press(&mut panel, KeyCode::Char(' '));
        assert_eq!(panel.coordinator, None);
        assert!(panel.validate().is_err());
    }
    #[test]
    fn restore_and_back_retain_fields_and_details_consume_keys() {
        let (mut panel, a, _) = setup();
        let draft = Draft {
            name: "Name".into(),
        ...
        panel.set_draft(draft.clone());
        panel.step = Step::Coordinator;
        press(&mut panel, KeyCode::F(2));
        press(&mut panel, KeyCode::Char(' '));
        assert!(panel.detail);
        press(&mut panel, KeyCode::Esc);
        assert!(panel.is_open());
    ...
    #[test]
    fn renders_every_stage_in_small_and_large_viewports_without_controls() {
        use ratatui::{Terminal, backend::TestBackend};
        let (mut panel, a, _) = setup();
        panel.helms[0].label = "unsafe\x1b[31m\x07".into();
        ...
        for (w, h) in [(1, 1), (8, 4), (24, 12), (120, 40)] {
    ...
    #[test]
    fn scope_limit_and_key_releases_are_enforced() {
        let (mut panel, _, _) = setup();
        panel.step = Step::Helms;
        panel.selected = (0..64).map(|_| Uuid::new_v4()).collect();
        press(&mut panel, KeyCode::Char(' '));
        assert_eq!(panel.selected.len(), 64);
        assert!(panel.error.is_some());
