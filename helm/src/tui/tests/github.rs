...
async fn github_panel_blocks_model_dispatch_and_session_navigation_until_closed() {
    ...
    assert!(
        crate::tui::commands::request_navigation(&mut app, &store, "other".into())
            .await
            .is_err()
    );
...
async fn publication_case(case: PublicationCase) {
    ...
    let server: tokio::task::JoinHandle<()> = tokio::spawn(async move {
        loop {
            ...
            let first = String::from_utf8_lossy(&request[..end])
                .lines()
                .next()
                .unwrap()
                .to_owned();
    ...
    let profiles = if case == PublicationCase::StalePolicy {
        ...
        let mut rules = Builtin::Autonomous.document().rules;
        rules.github_enabled = true;
        let snapshot = profiles
            .change(&ProfileChange {
                operation_id: Uuid::new_v4(),
                name: "github-fixture".into(),
                expected_revision: 0,
                action: Action::Create { rules },
            })
            .unwrap()
            .snapshot;
        ...
        let preview =
            Selection::preview(&config, app.session.workspace.as_path(), &selection).unwrap();
        config.policy_profile = Some(
            Selection::bind(
                &config,
                app.session.workspace.as_path(),
                selection,
        ...
        Some((
            profiles,
            Arc::new(crate::policy::Policy::new(&config, app.session.workspace.clone()).unwrap()),
        ))
    } else {
        None
    };
    let context = crate::tools::ToolContext {
        github: Some(crate::github::Credential::fixture("fixture-github-token")),
        completion: None,
        policy: profiles
            .as_ref()
    ...
    let service = crate::github::service::Service::fixture(
        context.clone(),
        Some(session),
        origin.clone(),
        store_path.clone(),
    ...
    let operation = service
        .prepare(crate::github::publication::Draft {
            object: crate::github::repository::Object::parse("https://github.com/o/r/issues/1")
                .unwrap(),
            action: crate::github::publication::Action::Comment { body },
        })
        .await
        .unwrap();
    ...
    let worker = tokio::spawn(async move {
        let result = service.publish(operation.id, &operation.digest).await;
        let _ = finished_tx.send(result);
    });
    app.github_panel
        .own_test_job(worker, context.cancellation.clone());
    ...
    let completed = tokio::time::timeout(Duration::from_secs(5), finished_rx)
        .await
        .unwrap();
    if case == PublicationCase::Cancelled {
        assert!(
            completed.is_err(),
            "closing the panel must abort its pending task"
        );
        assert!(context.cancellation.is_cancelled());
        assert!(!app.github_panel.open);
    }
    let result = completed.ok();
    let persisted = crate::github::store::Store::open(store_path.clone())
        .unwrap()
        .inspect(operation_id, &owner)
        .unwrap();
    if case == PublicationCase::Published {
        ...
        assert!(
            result.as_ref().is_none_or(|result| result.is_err()),
            "failure scenario must not claim publication"
        );
        assert!(persisted.receipt.is_none());
        assert_eq!(persisted.id, operation_id);
        assert_eq!(persisted.digest, digest);
        let expected = if case == PublicationCase::Disconnected {
            crate::github::store::State::Sending
        } else {
            crate::github::store::State::Prepared
        };
        assert_eq!(persisted.state, expected);
        assert_eq!(
            posts.load(Ordering::SeqCst),
            usize::from(case == PublicationCase::Disconnected)
        );
        if case == PublicationCase::Disconnected {
            let before = requests.load(Ordering::SeqCst);
            let reopened = crate::github::service::Service::fixture(
                context,
                Some(session),
                origin,
                store_path,
            ...
            assert_eq!(
                ...
                before,
                "uncertain recovery must not revalidate over HTTP or send again"
            );
            assert_eq!(posts.load(Ordering::SeqCst), 1);
            assert_eq!(
                reopened.inspect(operation_id).await.unwrap().state,
                crate::github::store::State::Sending
            );
            assert!(
                rx.try_recv().is_err(),
                "uncertain retry must not create another approval"
            );
        }
    }
