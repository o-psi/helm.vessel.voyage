...
use super::{
    publication::{Action, Draft, InlineComment, ReviewEvent, Side},
    repository::Object,
    service::Service,
    store::{State, Store},
};
...
fn review(event: ReviewEvent, invalid: u8) -> Draft {
    Draft {
        ...
        action: Action::Review {
            event,
            commit_id: HEAD.into(),
            body: if event == ReviewEvent::Approve {
                String::new()
            } else {
                "Exact review 🧭".into()
            },
...
async fn invalid_inline_locations_never_prepare_approve_or_publish() {
    for invalid in [1, 2, 3] {
        exercise(review(ReviewEvent::Comment, invalid), Reply::Success, true).await;
    }
}
#[tokio::test]
        ...
        exercise(
            Draft {
                ...
                action: Action::Comment {
                    body: "Exact issue-style comment".into(),
                },
            },
            reply,
            false,
...
async fn exercise(draft: Draft, reply: Reply, invalid: bool) {
    ...
    let server = tokio::spawn(async move {
        loop {
            ...
            let (status, response) = if route == "GET /user HTTP/1.1" {
                (200, json!({"id":7,"login":"fixture"}))
            } else if route == "GET /repos/o/r/pulls/1 HTTP/1.1" {
                (
                    200,
                    json!({"number":1,"html_url":URL,"state":"open","head":{"sha":HEAD},"base":{"sha":HEAD,"ref":"main","repo":{"id":1}}}),
                )
                ...
                assert_eq!(
                    route,
                    if is_review {
                        "POST /repos/o/r/pulls/1/reviews HTTP/1.1"
                    } else {
                        "POST /repos/o/r/issues/1/comments HTTP/1.1"
                    }
                );
                ...
                let (status, value) = match &action {
                    Action::Review {
                        ...
                        200,
                        json!({"id":9,"html_url":format!("{URL}#pullrequestreview-9"),"user":{"id":7},"body":body,"commit_id":commit_id,"state":match event {ReviewEvent::Comment=>"COMMENTED",ReviewEvent::Approve=>"APPROVED",ReviewEvent::RequestChanges=>"CHANGES_REQUESTED"}}),
                    ),
                    Action::Comment { body } => (
                        201,
                        json!({"id":9,"html_url":format!("{URL}#issuecomment-9"),"user":{"id":7},"body":body}),
                    ),
                };
                match reply {
                    ...
                    Reply::Forbidden => (
                        403,
                        json!({"message":"remote diagnostic must remain private"}),
                    ),
                    Reply::RateLimited => (
                        429,
                        json!({"message":"remote diagnostic must remain private"}),
    ...
    let context = ToolContext {
        github: Some(super::Credential::fixture(TOKEN)),
        completion: None,
    ...
    if invalid {
        ...
        assert!(
            service.list(0).await.unwrap().is_empty(),
            "invalid preparation persisted a draft"
        );
    } else {
        ...
        drop(service);
        let stored = Store::open(path.clone())
            .unwrap()
        ...
        if matches!(reply, Reply::Success) {
            ...
            assert!(!error.contains("remote diagnostic must remain private"));
        ...
        assert_eq!(
            ...
            before,
            "restart retransmitted or revalidated a terminal/uncertain send"
        );
        assert_eq!(posts.lock().unwrap().len(), 1);
        assert_eq!(approvals.lock().unwrap().len(), 1);
        assert_eq!(
