...
async fn refusal(case: Refusal) {
    ...
    let server = tokio::spawn(async move {
        loop {
            ...
            let (status, response) = if headers.starts_with("GET /user ") {
                ...
                (
                    200,
                    json!({"number":1,"html_url":"https://github.com/o/r/issues/1","state":"open"}),
                )
            } else {
                assert!(
                    headers.starts_with("POST /repos/o/r/issues/1/comments "),
                ...
                (
                    201,
                    json!({"id":9,"html_url":"https://github.com/o/r/issues/1#issuecomment-9","body":expected_body,"user":{"id":7,"login":"fixture"}}),
                )
            };
    ...
    assert_eq!(
        ...
        2,
        "preparation must use real account and object reads"
    );
    ...
    assert_eq!(
        ...
        0,
        "denied authority must not delegate to approver"
    );
    assert_eq!(posts.load(Ordering::SeqCst), 0);
    assert_eq!(
        requests.load(Ordering::SeqCst),
        2,
        "refusal must precede new HTTP requests"
    );
    if matches!(case, Refusal::ReadOnly) {
        let error = tokio::time::timeout(
            Duration::from_secs(5),
            service.prepare(operation.draft.clone()),
        )
        .await
        .unwrap()
        .expect_err("read-only Service preparation must refuse before network or mutation");
        assert!(
        ...
        let error = tokio::time::timeout(
            Duration::from_secs(5),
            service.cancel(operation.id, operation.digest.clone()),
        )
        .await
        .unwrap()
        .expect_err("read-only Service cancellation must preserve prepared state");
        assert!(
            error.to_string().contains("read-only"),
            "unexpected cancellation refusal: {error}"
        );
        assert_eq!(approvals.load(Ordering::SeqCst), 0);
        assert_eq!(requests.load(Ordering::SeqCst), 2);
        assert_eq!(posts.load(Ordering::SeqCst), 0);
    }
    let stored = Store::open(path)
        .unwrap()
    ...
    assert_eq!(
        ...
        before,
        "refusal must preserve exact prepared state"
    );
    stop.cancel();
