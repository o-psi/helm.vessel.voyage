...
use crate::{model::ToolDefinition,tools::{Tool,ToolContext,ToolError}};
...
enum Arguments {
    Read { request: super::context::Read },
    Prepare { draft: super::publication::Draft },
    Inspect { id: uuid::Uuid },
    List { #[serde(default)] offset:u32 },
...
impl Tool for GithubTool {
    ...
    async fn execute(&self, value:Value, context:&ToolContext) -> Result<String,ToolError> {
        ...
        match result {
            Ok(value) => service.project(&value,context.max_output_bytes.min(64*1024)).map_err(|_|ToolError::Failed("GitHub output budget is too small; use narrower context".into())),
