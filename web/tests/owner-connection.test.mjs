...
test('owner connection can choose a new folder; scoped connections retain their workspace list', async () => {
    ...
    field('settings-workspace').value='__custom__';
    field('settings-workspace').dispatchEvent(new Event('change'));
    assert.equal(field('settings-custom-workspace').hidden,false);
    field('settings-workspace-path').value='/new-folder';
    field('settings-workspace-path').dispatchEvent(new Event('input'));
    assert.equal(field('settings-save').disabled,true);
    field('settings-workspace-path').dispatchEvent(new Event('change'));
    await until(()=>!field('settings-save').disabled);
    ...
    field('settings-workspace-path').value='relative';
    field('settings-workspace-path').dispatchEvent(new Event('change'));
    assert.equal(seen.length,count);
    assert.match(field('settings-status').textContent,/absolute folder path/);
    scope='workspaces'; field('new-voyage').click();
    await until(()=>field('settings-workspace').value==='/known');
    assert.equal(field('settings-custom-workspace').hidden,true);
    assert.ok(![...field('settings-workspace').options].some(o=>o.value==='__custom__'));
    dom.window.close();
});
