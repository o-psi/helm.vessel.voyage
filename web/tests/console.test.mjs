...
test('browser journey: history, live output, submit, approval, question, cancel, reconnect receipts without replay', {timeout:120000}, async (t) => {
    ...
    try {
        ...
        assert.equal($('#edit-profile').value,'personal','a saved profile is selected in the Flux control');
        assert.match($('#edit-profile-summary').textContent,/fixture/);
        const setupSubmits=requests.filter(c=>c.op==='submit'||c.op==='steer').length;
        $('#edit-profile').dispatchEvent(new dom.window.KeyboardEvent('keydown',{key:'Enter',bubbles:true,cancelable:true}));
        ...
        $('#change-setup').click();
        assert.equal($('#edit-profile ui-option[data-settings-option]'),null,'reloading Setup removes stale selectable profiles');
        ...
        $('#setup-profile-open').click();
        await until(()=>!$('#setup-profiles').hidden && $('#edit-profile ui-option[data-settings-option]'));
        const profileOption=name=>[...root.querySelectorAll('#edit-profile ui-option[data-settings-option]')].find(option=>option.querySelector('[data-option-label]')?.textContent.startsWith(name));
        $('#edit-profile').value=profileOption('Work').getAttribute('value');
        $('#edit-profile').dispatchEvent(new Event('change',{bubbles:true}));
        $('#edit-save').click();
        ...
        $('#edit-profile').value=profileOption('Everyday').getAttribute('value');
        $('#edit-profile').dispatchEvent(new Event('change',{bubbles:true}));
        $('#setup-back').click();
