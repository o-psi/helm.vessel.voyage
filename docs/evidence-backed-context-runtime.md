...
The components sum to total bytes. Chat-format system messages remain inside history; this coarse breakdown is not semantic per-role attribution. Events indicate request construction, **not dispatch, provider usage or billing**. This slice does not add durable per-attempt ledger fields, cache/reasoning token counters or task-wide cost aggregation. OAuth measurements occur after its request-body adaptations. Auth/device/token requests are excluded.
...
The implementation intentionally does **not** add learned compression, a competing todo/completion state machine, schema loading, structured filter/group, new sharing permissions or unlimited retention. Still required for the full design: durable detailed per-attempt accounting, representative behavioral benchmarks/cache measurements, evidence-linked task-state integration, broader quota/crash/branch/revocation tests and approved rollout decisions. No live-provider token/cost improvement or native Windows/macOS result is claimed. Keep #282 open for this remaining scope.
