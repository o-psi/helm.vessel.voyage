...
- Main is at 2f4c89c8: the full local completion/managed stack (#88), saved workflow CLI (#123), and runtime administrator ceiling (#124) are delivered with passing Linux baselines and CI. Issues #81 and #66 are closed after acceptance review.
- Onboarding correction [#125](https://github.com/o-psi/voyage/pull/125), headb2d0385, includes the policy runtime main and passes832 workspace tests normally and under two CPUs; release/system/CI checks continue. An inherited WaitFailed fixture now directly observes the injected error before async retry, avoiding its old10ms scheduling race; all assertions are preserved and20 exact two-CPU repetitions pass. It preserves existing instructions and omits command candidates when bounded guidance inspection finds conflicts or uncertainty.
...
- Saved workflows #67 retain TUI parameter entry and secret-input work. Completion #82/#83 retain remote output and behavioral evidence. The latest bounded local evaluation failed honest accounting and timed out; the separate subscription path returned HTTP401. A separate guided #80 local-model questions smoke is active with a combined12-request/360-second cap across attempts,512outputtokens/request, isolatedHOME/XDG and verifiedb2d0385 release snapshot. No paid endpoints. First attempt was interrupted during one incomplete model response; the initial path-error diagnosis was mistaken and corrected, with no acceptance claimed. Current evidence: `.local-git/evidence/questions-local-live/`.
