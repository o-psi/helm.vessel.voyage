...
Today `helm --voyage [VESSEL_URL] --name NAME` requests a short-lived pairing code;
an operator claims it using `vessel pair STRING`. Helm persists enrollment and uses
authenticated outbound HTTP heartbeats, task polling, and results. Queued task
requests can reference a saved session, but this is not interactive session management.
The current console manages fleet and tasks, not the session surface specified here.
See [architecture](architecture.md), [Helm usage](../helm/README.md), and
[Vessel usage](../vessel/README.md) for existing commands.

The target reverses enrollment: Vessel creates an invitation and Helm redeems it.
Preserve existing pairing/task clients until an explicit, tested migration path is
available. Do not silently reinterpret old pairing strings as join keys.
...
independent of transport. Existing HTTP polling remains current until replaced or
augmented compatibly. Session commands are distinct from queued one-shot tasks.
...
Unit and integration fixtures must cover enrollment expiry/replay/races, old/new
protocol compatibility, local/remote contention, duplicate commands, disconnect at
