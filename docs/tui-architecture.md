...
- While a top-level run is active, ordinary composer input is bounded steering rather than a
  second concurrent run. Accepted messages are saved immediately and enter the agent history in
  FIFO order at the next provider boundary; the queue closes atomically with completion.
