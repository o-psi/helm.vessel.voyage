...
`npm ci --prefix voyage/browser --ignore-scripts --omit=dev --no-audit --no-fund`
before packaging. The Linux packager inventories `voyage/browser/*.mjs`, the Python guardian, package
metadata and pinned `playwright-core` under `share/voyage/browser`; the installer
verifies and preserves their hashes as part of release identity. It refuses
