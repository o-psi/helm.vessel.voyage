...
impl Journal {
    ...
    pub(crate) fn attest_session_resource(
        ...
        let state: String = tx.query_row(
            ...
            state == "cleanup_unknown" || state == "operator_attested",
    ...
    pub(crate) fn session_resources(&self, session: Uuid) -> Result<Value> {
        let mut query=self.connection.prepare("SELECT id,run_id,kind,state FROM process_session_resources WHERE session_id=?1 AND state NOT IN ('observed','operator_attested') ORDER BY rowid LIMIT 256")?;
...
pub(super) fn pending(db: &Connection, session: Uuid) -> Result<i64> {
    ...
    Ok(db.query_row("SELECT count(*) FROM process_session_resources WHERE session_id=?1 AND state NOT IN ('observed','operator_attested')",[session.to_string()],|r|r.get(0))?)
}
