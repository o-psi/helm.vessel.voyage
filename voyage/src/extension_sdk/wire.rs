...
pub(crate) fn parse_json(bytes: &[u8]) -> Result<Value> {
    ...
    let value = Seed { depth: 0, nodes: &mut nodes }.deserialize(&mut decoder)
        .map_err(|_| anyhow::anyhow!("invalid or excessive extension JSON"))?;
    decoder.end().map_err(|_| anyhow::anyhow!("trailing extension JSON"))?;
    Ok(value)
}

struct Seed<'a> { depth: usize, nodes: &'a mut usize }
impl<'de> DeserializeSeed<'de> for Seed<'_> {
    type Value = Value;
    fn deserialize<D: serde::Deserializer<'de>>(self, de: D) -> Result<Value, D::Error> {
        if self.depth > 48 || *self.nodes == 0 { return Err(D::Error::custom("JSON limit")); }
        *self.nodes -= 1;
...
impl<'de> Visitor<'de> for Seed<'_> {
    ...
    fn visit_f64<E: Error>(self, v: f64) -> Result<Value,E> {
        Number::from_f64(v).map(Value::Number).ok_or_else(|| E::custom("invalid number"))
    }
    fn visit_str<E: Error>(self, v: &str) -> Result<Value,E> { Ok(Value::String(v.into())) }
    ...
    fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Value,A::Error> {
        ...
        while let Some(value) = seq.next_element_seed(Seed { depth: self.depth+1, nodes: self.nodes })? {
            values.push(value);
        }
    ...
    fn visit_map<A: MapAccess<'de>>(self, mut map: A) -> Result<Value,A::Error> {
        ...
        while let Some(key) = map.next_key::<String>()? {
            if values.contains_key(&key) { return Err(A::Error::custom("duplicate key")); }
            let value = map.next_value_seed(Seed { depth: self.depth+1, nodes: self.nodes })?;
...
pub(super) async fn read<R: AsyncBufRead + Unpin + ?Sized>(reader: &mut R) -> Result<Value> {
    ...
    loop {
        ...
        if chunk.is_empty() { bail!("extension EOF before complete frame"); }
        let newline = chunk.iter().position(|b| *b == b'\n');
        let take = newline.map_or(chunk.len(), |i| i + 1);
        ensure!(bytes.len() + take <= MAX_FRAME + 1, "extension frame exceeds limit");
        bytes.extend_from_slice(&chunk[..take]);
        reader.consume(take);
        if newline.is_some() { break; }
    }
    bytes.pop();
    ensure!(!bytes.is_empty() && !bytes.contains(&b'\r'), "invalid NDJSON framing");
    parse_json(&bytes)
}

pub(super) async fn write<W: AsyncWrite + Unpin + ?Sized, T: Serialize>(writer: &mut W, value: &T) -> Result<()> {
