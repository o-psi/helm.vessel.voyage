...
impl InvocationHandle {
    ...
    pub(crate) async fn completion(mut self) -> Completion {
        match self.receiver.take().expect("completion receiver").await {
            ...
            Err(_) => Completion {
                result: Err(anyhow::anyhow!("extension supervisor interrupted")),
...
impl Executor {
    pub(crate) fn new(
        ...
        Ok(Arc::new(Self {
            ...
            admitted: Arc::new(Semaphore::new(9)),
    ...
    /// Task drain alone is not child cleanup. The concrete parent adapter may
    /// combine it with positive observation of every retained child/reservation.
    pub(crate) fn tasks_drained(&self) -> bool {
        self.closed.is_cancelled() && self.admitted.available_permits() == 9
    }
    ...
    pub(crate) fn start(
        ...
        host: Arc<dyn Host>,
            ...
            "extension executor is quarantined or closed"
        );
        ...
        let output = CompiledSchema::compile(&def.output_schema)?;
            ...
            call.identity.package.len() <= 128
                && !call.identity.package.is_empty()
                && call
                    .identity
                    .package
                    .bytes()
        ...
        call.deadline = call.deadline.min(Instant::now() + MAX_INVOCATION);
            ...
            call.deadline > Instant::now() && !call.cancellation.is_cancelled(),
            "extension admission expired"
        );
        let admission = self
            .admitted
            .clone()
        ...
        tokio::spawn(async move {
            ...
            let completion = if slot.is_some() {
                ...
                Completion {
                    result: Err(anyhow::anyhow!("extension queue cancelled or expired")),
                    cleanup: Cleanup::Observed,
                }
            };
            if completion.cleanup == Cleanup::Observed {
                executor.unsettled.fetch_sub(1, Ordering::SeqCst);
            }
            if slot.is_some()
                && (completion.result.is_err() || completion.cleanup == Cleanup::Pending)
            {
                executor.close();
            }
    ...
    async fn execute(
        ...
        let mut launched = match launched {
            ...
            Err(_) => {
                return Completion {
                    result: Err(anyhow::anyhow!("extension launch refused or interrupted")),
                    cleanup: Cleanup::Pending,
                };
            }
        };
        ...
        if result.is_err() {
            let _ = timeout(
                ...
                wire::write(
                    &mut launched.writer,
                    &json!({"type":"cancel","invocation":call.identity.invocation}),
                ),
            )
            .await;
        }
        let shutdown = timeout(SHUTDOWN, async {
            ...
            let mut byte = [0];
                ...
                reader.read(&mut byte).await? == 0,
                "late extension response"
            );
            Ok::<_, anyhow::Error>(())
        })
        .await;
        ...
        let cleanup = timeout(CLEANUP, launched.lease.terminate_and_observe())
            .await
    ...
    async fn exchange<R: tokio::io::AsyncBufRead + Unpin, W: AsyncWrite + Unpin>(
        ...
        timeout(INIT, async {
            wire::write(
                writer,
                &json!({"type":"initialize","protocol":1,"identity":call.identity,
                "definitions":self.pinned,"capabilities":self.capabilities}),
            )
            .await?;
                ...
                Incoming::Initialized {
                    ...
                    capabilities,
                        ...
                        protocol == 1
                            ...
                            && definitions == self.pinned
                            && capabilities == self.capabilities,
                        "extension initialization mismatch"
                    );
                    Ok(())
                }
        ...
        .await
        .map_err(|_| anyhow::anyhow!("extension initialization expired"))??;
        wire::write(
            writer,
            &json!({"type":"invoke","invocation":call.identity.invocation,
            "kind":call.kind,"name":call.name,"arguments":call.arguments}),
        )
        .await?;
        ...
        loop {
            match decode(wire::read(reader).await?)? {
                Incoming::Progress { invocation, text } => {
                        ...
                        invocation == call.identity.invocation && text.len() <= 4096,
                        "invalid extension progress"
                    );
                    let now = Instant::now();
                    while progress
                        .front()
                    ...
                    progress_count += 1;
                        ...
                        progress_count <= 256 && progress.len() < 10,
                        "extension progress rate exceeded"
                    );
                    progress.push_back(now);
                    host.progress(&call.identity, &text)
                        .await
                        .map_err(|_| anyhow::anyhow!("extension progress refused"))?;
                }
                Incoming::Result { invocation, value } => {
                    ensure!(
                        invocation == call.identity.invocation,
                        "extension result identity mismatch"
                    );
                ...
                Incoming::Error { invocation, code } => {
                        ...
                        invocation == call.identity.invocation,
                        "extension error identity mismatch"
                    );
                ...
                Incoming::HostRead {
                    ...
                    requests += 1;
                        ...
                        self.capabilities.iter().any(|c| c == "host.file.read"),
                        "extension host capability denied"
                    );
                    ensure!(
                        !path.is_empty()
                            && path.len() <= 4096
                            ...
                            && max_bytes > 0
                            && max_bytes <= MAX_READ,
                        "invalid extension host read bounds"
                    );
                    ...
                    let reply = match read {
                        Ok(text) => {
                                ...
                                text.len() <= max_bytes,
                                "extension broker exceeded read bound"
                            );
                            json!({"type":"host_result","invocation":invocation,"request":request,"text":text})
                        }
                        Err(_) => {
                            json!({"type":"host_error","invocation":invocation,"request":request,"code":"denied"})
                        }
                    };
...
mod tests {
    ...
    impl LaunchAdapter for Adapter {
        async fn launch(&self, _: &Identity, _: Instant) -> Result<Launched> {
            ...
            tokio::spawn(async move {
                ...
                wire::write(
                    &mut write,
                    &json!({"type":"result", "invocation":call["invocation"], "value":{"ok":true}}),
                )
                .await
                .unwrap();
                let shutdown = wire::read(&mut read).await.unwrap();
                assert_eq!(shutdown["type"], "shutdown");
                wire::write(&mut write, &json!({"type":"shutdown_ack"}))
                    .await
                    .unwrap();
            ...
            Ok(Launched {
                ...
                lease: Box::new(TestLease(self.cleanup.clone())),
            })
        }
    }
    struct NoHost;
    ...
    impl Host for NoHost {
        ...
        async fn progress(&self, _: &Identity, _: &str) -> Result<()> {
            Ok(())
        }
    }
    fn pinned() -> Value {
    ...
    fn strict_response_fields_and_codes() {
        ...
        assert!(
            decode(json!({"type":"error","invocation":Uuid::new_v4(),"code":"private secret"}))
                .is_err()
        );
    }
    ...
    async fn queue_is_bounded_and_close_drains_without_late_launches() {
        ...
        timeout(Duration::from_secs(1), adapter.entered.notified())
            .await
            .unwrap();
        let queued: Vec<_> = (0..8)
            .map(|_| executor.start(call(), Arc::new(NoHost)).unwrap())
            .collect();
        ...
        for handle in queued {
            assert!(handle.completion().await.result.is_err());
        }
