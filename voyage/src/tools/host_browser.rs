    ...
    async fn execute(&self, arguments: Value, context: &ToolContext) -> Result<String, ToolError> {
        ...
        if bytes_output {
            let encoded = value["data_base64"]
                .as_str()
                .ok_or_else(|| failed("missing artifact"))?;
            if encoded.len() > 3 * 1024 * 1024 {
                return Err(failed("artifact bound"));
            }
            ...
            let scope = context
                .artifact_scope
                .as_ref()
                .ok_or_else(|| failed("artifact scope"))?;
            let mut store =
                crate::artifacts::Store::open(&scope.directory, scope.session).map_err(failed)?;
            let meta = store
                .put(
                    Uuid::new_v4(),
                    value["name"].as_str().unwrap_or("browser-capture.jpg"),
                    value["mime_type"]
                        .as_str()
                ...
                .map_err(failed)?;
            return serde_json::to_string(&json!({"artifact":meta})).map_err(failed);
        }
