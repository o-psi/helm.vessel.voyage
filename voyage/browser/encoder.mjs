...
export function encoderRuntime() {
  ...
  globalThis.encoder={
    ...
    async frame({data,width,height,generation}){
      ...
      try{if(generation!==epoch)return;if(canvas.width!==width||canvas.height!==height){canvas.width=width;canvas.height=height;}context.drawImage(bitmap,0,0,width,height);track.requestFrame();}finally{bitmap.close();}
    },
    async reset(generation){epoch=generation;for(const p of peers.values())p.close();peers.clear();context.clearRect(0,0,canvas.width,canvas.height);track.requestFrame();},
    async offer({viewer,iceServers,relayOnly}){
      peers.get(viewer)?.close();const pc=new RTCPeerConnection({iceServers,iceTransportPolicy:relayOnly?'relay':'all',bundlePolicy:'max-bundle'});peers.set(viewer,pc);
    ...
    stop(){for(const pc of peers.values())pc.close();peers.clear();stream.getTracks().forEach(t=>t.stop());},
  };
}
