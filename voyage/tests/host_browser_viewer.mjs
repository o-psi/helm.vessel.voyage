...
async function mode(page, label, expected, media=true){
 ...
 await page.waitForFunction(expected=>document.querySelector('[role=status]')?.textContent.includes(`· ${expected} control`) && !document.querySelector('button').disabled,expected);
 if(media)await decoded(page,expected);
}
async function navigate(page,url){
 await page.getByRole('textbox',{name:'Navigate browser',exact:true}).fill(url);
...
async function native(item,observer){
 ...
 await page.getByRole('button',{name:'Start / Connect',exact:true}).waitFor();
 assert.equal(new URL(page.url()).hash,'','bootstrap must remove launch secret from history');
 await page.getByRole('button',{name:'Start / Connect',exact:true}).click();
 await decoded(page,'native '+item.native_route);
 await mode(page,'Take control','human');
 ...
 await page.getByRole('button',{name:'Disconnect viewer',exact:true}).click();
...
async function suspendedNative(item){
 ...
 try{
  await page.goto(new URL('file://'+item.launcher).href);
  await page.getByRole('button',{name:'Start / Connect',exact:true}).click();
  await page.waitForFunction(()=>window.operations.some(o=>o.action==='start'));
  ...
  await mode(page,'Take control','human',false);
...
try{
 ...
 for(const p of [a,b]){
  const page=await browser.newPage();p.page=page;
  await page.exposeFunction('hostTransport',async operation=>{const result=await p.command({op:'host_browser',operation});if(result.status?.binding)p.binding=result.status.binding;return result;});
  await page.goto(cfg.site+'/receiver');
  ...
  await page.getByRole('button',{name:'Start / Connect',exact:true}).click();
  await decoded(page,'mounted '+p.item.label);
 }
 await a.page.bringToFront();
 // Reconnect after document-hidden clears the prior viewer's media.
 await a.page.getByRole('button',{name:'Start / Connect',exact:true}).click();
 await decoded(a.page,'mounted reconnect');
 await mode(a.page,'Take control','human');
 await navigate(a.page,cfg.site+'/human');
 await mode(a.page,'Private control','private');
 await navigate(a.page,cfg.site+'/private');
 await a.page.getByRole('textbox',{name:'Text for remote browser',exact:true}).fill('SYNTHETIC_PRIVATE_INPUT_333');
 await a.page.getByRole('button',{name:'Send text',exact:true}).click();
 await a.page.waitForFunction(()=>!window.mounted.session.pumping);
 assert.equal(await a.page.evaluate(()=>window.mounted.session.status?.mode),'private');
 assert.equal((await b.op('status')).status.mode,'agent');
 await mode(a.page,'Return to agent','agent');
 for(const p of [a,b]){await p.page.evaluate(()=>window.mounted.dispose());await p.page.close();}
